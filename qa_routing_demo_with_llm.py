"""
QA ROUTING DEMO WITH LLM & DEEPEVAL
=====================================

This is a production-ready implementation that combines:
1. RAG (Retrieval-Augmented Generation) using FAISS
2. LLM (Large Language Model) using Groq API
3. DeepEval for quality measurement

Key Features:
- Uses free Groq API (no quota issues)
- FAISS for vector search (local, no external DB needed)
- DeepEval metrics for quality evaluation
- Handles all 4 flows: Known Answer, Escalation, General Chat, Out-of-Scope
- Fully commented for learning purposes

Requirements:
- pip install groq faiss-cpu sentence-transformers deepeval

Setup:
1. Get Groq API key from https://console.groq.com
2. Set environment variable: export GROQ_API_KEY="your-key"
3. Run: python qa_routing_demo_with_llm.py
"""
...
# Import DeepEval for quality evaluation
...
class RAGConfig:
    ...
    EVALUATE_ANSWERS = False  # Set to False if no OpenAI key
    EVALUATION_THRESHOLD = 0.75  # Minimum acceptable score
...
class EmbeddingEngine:
    ...
    How it works:
    1. Convert questions to embeddings (vectors)
    2. Store vectors in FAISS index
    3. For a new question, convert to embedding
    4. Search FAISS index for similar vectors
    5. Return top-k most similar questions
    """
    ...
    def build_index(self, questions: List[str]) -> None:
        ...
        - Pre-computing embeddings saves time during inference
        - FAISS index enables fast similarity search
        - Without this, each search would be slow
        """
    ...
    def search(self, query: str, k: int = 3) -> List[Tuple[int, float]]:
        ...
        Returns:
            ...
            - index: Index of the most similar Q&A pair
        ...
        # Return results (flatten from batch format)
...
class QueryClassifier:
    ...
    def __init__(self, client: Groq):
        ...
        self.client = client
        self.model = RAGConfig.GROQ_MODEL
    ...
    def classify(self, question: str) -> Tuple[str, float]:
        ...
        Returns:
            ...
            - category: QA_SPECIFIC, GENERAL_CHITCHAT, or OUT_OF_SCOPE
...
class AnswerGenerator:
    ...
    How it works:
    1. Retrieve relevant Q&A from knowledge base
    2. Use retrieved context to prompt LLM
    3. LLM generates intelligent answer based on context
    4. This prevents hallucination (answer is grounded in KB)
    """
    
    def __init__(self, client: Groq, kb_loader: KnowledgeBaseLoader):
    ...
    def generate(self, question: str, context_qa: Dict) -> str:
        ...
        Args:
            question: User's question
            context_qa: Retrieved Q&A pair from knowledge base
        
        Returns:
            Generated answer
        """
        
        # System prompt tells LLM to use context
...
class QualityEvaluator:
    ...
    - Faithfulness: Is the answer based on the context (not hallucinated)?
    
    Why evaluation?
    - Measures quality objectively
    - Identifies problematic answers
    ...
    def evaluate(self, question: str, answer: str, context: str) -> Dict:
        ...
        try:
            ...
            # Calculate average
            avg_score = (relevancy_score.score + faithfulness_score.score) / 2
            
            return {
                "relevancy": relevancy_score.score,
                "faithfulness": faithfulness_score.score,
                "average": avg_score,
                "pass": avg_score >= RAGConfig.EVALUATION_THRESHOLD
            }
...
class EscalationHandler:
    ...
    Why escalation?
    - Some questions can't be answered from KB
    - Need human review for new/complex issues
    - Escalated questions are saved for future learning
    """
...
class QAChatbot:
    ...
    Flow:
    1. Classify question (QA_SPECIFIC, GENERAL_CHITCHAT, OUT_OF_SCOPE)
    2. If QA_SPECIFIC: Retrieve from KB and generate answer
    3. If GENERAL_CHITCHAT: Return friendly response
    4. If OUT_OF_SCOPE: Return polite refusal
    5. Evaluate answer quality with DeepEval
    6. If quality is low: Escalate to QA team
    """
    
    def __init__(self):
        """Initialize chatbot"""
        print("\n" + "="*70)
        print("QA ROUTING DEMO WITH LLM & DEEPEVAL")
        print("="*70)
        ...
        self.classifier = QueryClassifier(self.groq_client)
        self.generator = AnswerGenerator(self.groq_client, self.kb_loader)
        self.evaluator = QualityEvaluator() if RAGConfig.EVALUATE_ANSWERS else None
    ...
    def process_question(self, question: str) -> Dict:
        ...
        if category == "GENERAL_CHITCHAT":
            ...
            print(f"   Similarity: {1 - similarity:.2f}")  # Convert distance to similarity
            
            # Threshold for "known answer"
