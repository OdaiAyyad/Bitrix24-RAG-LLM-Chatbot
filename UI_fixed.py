"""
QA ROUTING CHATBOT - STREAMLIT UI VERSION (FIXED)
==================================================

FIXES APPLIED:
1. Chat input now always visible (doesn't disappear after clicking example buttons)
2. Flow 1 answer generation now properly displays the answer
...
def process_question(user_question: str, qa_pairs: list, model: str):
    """
    Process a question and return the response
    
    FIX: This function now properly handles Flow 1 answer generation
    """
    ...
    if category == "GENERAL_CHITCHAT":
        ...
        if similarity > 0.3:
            flow = 1
            # FIX: Properly generate and assign the answer
...
def main():
    ...
    with col1:
        ...
        # FIX #1: Check if there's a pending question from example buttons
        # Process it immediately without hiding the chat input
        if "pending_question" in st.session_state and st.session_state.pending_question:
            ...
            with st.spinner("🔄 Processing..."):
                result = process_question(user_question, qa_pairs, model)
            
            # Display bot response
            with st.chat_message("assistant"):
                # Flow badge
                st.markdown(f"<div style='background-color: {result['flow_color']}; color: white; padding: 10px; border-radius: 5px; margin-bottom: 10px;'>{result['flow_text']}</div>", unsafe_allow_html=True)
                
                # Confidence
                st.markdown(f"**Classification**: {result['category']} (Confidence: {result['confidence']:.0%})")
            ...
            st.session_state.messages.append({
                "role": "assistant",
                "content": f"{result['flow_text']}\n\n{result['answer']}"
            })
            
            # Rerun to show the updated chat and chat input
            st.rerun()
        
        # FIX #1: Chat input is ALWAYS shown, not conditionally
    ...
    with col2:
        ...
        for i, q in enumerate(example_questions):
            if st.button(q, key=f"example_{i}"):
                # Set pending question and rerun
